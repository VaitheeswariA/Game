...
public class GoldTPM {
	...
	public static void main(String[] args) {
		...
		System.out.println("Enter trigger position (row and column) : ");
		...
		int monsterSteps = pathMap.size()>0 ? pathMap.firstKey():0;
		pathMap.clear();
		...
		if(advSteps > monsterSteps) {
			findMinimumPossiblePaths(visited,advRow,advCol,triggerRow,triggerCol,0,"");
			triggerSteps= pathMap.size()>0 ? pathMap.firstKey():0;
			pathMap.clear();
			
			findMinimumPossiblePaths(visited,triggerRow,triggerCol,goldRow,goldCol,0,"");
			advSteps = pathMap.size()>0 ? pathMap.firstKey():0;
			advSteps+=triggerSteps;
			pathMap.clear();
		}
